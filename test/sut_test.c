...
{
	sut_formatter_t *tap_formatter = malloc(sizeof *tap_formatter);
...
sut_test_t *
sut_test_new(char *name, void (*function)(void))
{
	sut_test_t *test = malloc(sizeof *test);
	test->name = name;
...
sut_suite_t *
sut_suite_new(char *name)
{
	return sut_suite_new_formatted(name, sut_formatter_tap(stdout));
}

sut_suite_t *
sut_suite_new_formatted(char *name, sut_formatter_t *formatter)
{
	sut_suite_t *suite = malloc(sizeof *suite);
...
{
	if (suite && test) {
		sut_test_elem_t *e = calloc(1, sizeof *e);
		e->test = test;
...
_sut_fork_and_run_test(sut_test_t *test)
{
	...
	if ((childpid = fork()) == -1) {   // error
		...
		if ((shmresult = shmat(shmid, NULL, 0)) == (void *) -1) {
		...
		while (true) {
			...
			if (endpid == -1) {
				...
				if (WIFSIGNALED(status) || WIFSTOPPED(status)) {
					test->error = malloc(sizeof test->error);
					test->error->reason = malloc(sizeof(char) * 256);

					if (WIFSIGNALED(status)) {
						snprintf(test->error->reason, 255,
		...
		test->start = malloc(sizeof test->start);
		*test->start = shmresult->start;

		test->end = malloc(sizeof test->end);
...
unsigned int
sut_suite_run(sut_suite_t *suite, char *filter)
{
